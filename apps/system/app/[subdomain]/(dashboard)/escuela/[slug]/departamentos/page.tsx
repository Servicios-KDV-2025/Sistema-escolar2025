...
function DepartamentoFormIntegrado({ form }: { form: UseFormReturn<DepartamentoFormValues> }) {
  return (
          ...
          <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
            <div className="space-y-0.5">
              ...
              <div className="text-xs font-medium">
                Estado actual: <span className={field.value ? "text-green-600" : "text-red-600"}>
            ...
            <FormControl>
              <Switch
                ...
                onCheckedChange={(val) => {
                  field.onChange(val);
                  console.log('Estado cambiado a:', val);
                }}
                disabled={false}
              />
            </FormControl>
...
export default function DepartamentosPage() {
  ...
  // Usar el store de departamentos
  const {
    departamentos,
  ...
  const { isOpen, operation, data, openCreate, openEdit, openView, openDelete, close } =
    useCrudDialog(departamentoSchema, { nombre: '', descripcion: '', activo: false });
